...
export default function CategoriesPage() {
  ...
  return (
    <DashboardLayout navItems={wholesalerNavItems}>
      ...
      <form onSubmit={handleAdd} className="bg-white rounded-lg shadow p-4 mb-6 flex gap-2">
        <input
          type="text" placeholder="New category name" value={newName}
      ...
      <div className="bg-white rounded-lg shadow divide-y">
        {isLoading ? (
          ...
          <p className="p-4 text-sm text-gray-400">No categories yet — add one above.</p>
        ) : (
          categories.map((c) => (
            <div key={c.category_id} className="flex justify-between items-center p-4">
              <span className="text-sm font-medium text-[#003049]">{c.category_name}</span>
              <button onClick={() => setPendingDelete(c)} className="text-gray-400 hover:text-[#d62828]">
                <Trash2 size={16} />
              </button>
            </div>
          ))
